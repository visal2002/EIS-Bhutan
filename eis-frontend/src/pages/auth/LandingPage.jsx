...
import {
    Zap, Droplets, Factory, Leaf, ArrowRight, BarChart3,
    TrendingUp, Globe, ChevronRight, Activity,
    Shield, Database, Download, Info, ArrowUpRight, Flame,
} from "lucide-react";
...
const NC = {
    ...
    "Petroleum (Imported)":"#b45309","Electricity Grid":"#2563eb",
    "Coal Processing":"#64748b","Petroleum Distribution":"#f59e0b",
    "Biomass Processing":"#65a30d","Residential":"#6366f1",
    "Commercial":"#8b5cf6","Industry":"#7c3aed","Transport":"#ea580c",
    "Other Sectors":"#94a3b8","Exports":"#f43f5e","Losses / Stock Adj.":"#cbd5e1",
};

const HERO_IMGS = ["/images/hero1.jpg","/images/hero2.jpg","/images/hero3.jpg"];
const TREND = [
    {y:"2010",hydro:180000,biomass:72000,petroleum:68000},
    {y:"2012",hydro:198000,biomass:76000,petroleum:79000},
    {y:"2014",hydro:210000,biomass:80000,petroleum:88000},
    {y:"2016",hydro:222500,biomass:84693,petroleum:97500},
    {y:"2018",hydro:248000,biomass:90000,petroleum:108000},
    {y:"2020",hydro:272000,biomass:96000,petroleum:115000},
    {y:"2022",hydro:301852,biomass:100617,petroleum:130000},
];
const SECTORS = [
    {icon:Zap,       label:"Electricity",  desc:"Generation, transmission & consumption across all 20 dzongkhags.",      color:"#2563eb",bg:"#eff6ff"},
    {icon:Droplets,  label:"Petroleum",    desc:"Import, export & distribution of all petroleum and POL products.",       color:"#b45309",bg:"#fef3c7"},
...
const INTEG = [
    {a:"NDI",f:"National Digital Identity"},{a:"FIRMS",f:"Forest Info System"},
    {a:"eRALIS",f:"Vehicle Registration"},{a:"MAS",f:"Mining Admin System"},
    {a:"IIS",f:"Industry Info System"},{a:"OFS",f:"Online Filing System"},
];
...
function CountUp({end,suffix=""}) {
    ...
    useEffect(()=>{
        const o=new IntersectionObserver(([e])=>{
            ...
            const run=now=>{const p=Math.min((now-t0)/1600,1);setV(Math.round((1-Math.pow(1-p,3))*end));if(p<1)requestAnimationFrame(run);};
...
export default function LandingPage() {
    ...
    useEffect(()=>{const t=setInterval(()=>setImgIdx(i=>(i+1)%HERO_IMGS.length),4500);return()=>clearInterval(t);},[]);
    const nd=nationalData[year],ed=energyData[year];
        ...
        <div className="min-h-screen flex flex-col bg-slate-50 antialiased" style={{fontFamily:"system-ui,sans-serif"}}>
            ...
            <section className="relative min-h-screen flex items-center overflow-hidden" style={{background:`linear-gradient(135deg,${P.navy} 0%,#0d2a1f 60%,#1a1a2e 100%)`}}>
                ...
                <div className="absolute right-0 top-0 bottom-0 w-[55%] overflow-hidden hidden lg:block">
                    {HERO_IMGS.map((src,i)=>(
                        <img key={src} src={src} alt="" className={"absolute inset-0 w-full h-full object-cover transition-opacity duration-[2000ms] "+(i===imgIdx?"opacity-100":"opacity-0")}/>
                    ))}
                ...
                <div className="relative z-10 w-full max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-14 xl:px-20 pt-24 pb-16">
                    <div className="max-w-xl lg:max-w-2xl">
                        ...
                        <p className="text-white/60 text-[1.1rem] leading-relaxed max-w-lg mb-10">
                            Bhutan&apos;s unified platform for energy data collection, cross-sector analytics,
                            and IPCC-aligned GHG reporting — covering all 20 dzongkhags and 9 energy streams.
                        </p>
                        <div className="flex flex-wrap gap-4 mb-12">
                            <button onClick={()=>nav("/public")} className="flex items-center gap-2.5 px-7 py-3.5 rounded-full text-sm font-bold transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5"
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {[[Zap,"Electricity"],[Droplets,"Petroleum"],[Leaf,"Biomass"],[Factory,"Industry"],[BarChart3,"GHG"],[Activity,"Solar"]].map(([Icon,label])=>(
            ...
            {/* UPGRADED: SANKEY — dark section */}
            <section className="py-0 border-b border-slate-900 relative overflow-hidden" style={{background:"linear-gradient(160deg,#080e1c 0%,#0c1e14 55%,#080e1c 100%)"}}>
                ...
                <div className="relative z-10 max-w-[1320px] mx-auto px-6 lg:px-10 pt-16 pb-8">
                    ...
                    <div className="flex items-center gap-4 flex-wrap mb-5">
                        ...
                        {[["#15803d","Domestic"],["#0284c7","Imports"],["#3e6c7e","Hydro"],["#2563eb","Elec. Grid"],["#65a30d","Biomass"],["#b45309","Petroleum"],["#7c3aed","Industry"],["#f43f5e","Exports"]].map(([c,l])=>(
                            <span key={l} className="flex items-center gap-1.5 text-[10px] font-semibold" style={{color:"rgba(255,255,255,0.45)"}}>
            ...
            <section className="py-20 bg-white border-b border-slate-100">
                <div className="max-w-[1320px] mx-auto px-6 lg:px-10">
                        ...
                        <div className="rounded-3xl overflow-hidden shadow-lg" style={{background:"linear-gradient(160deg,#0d1f14,#0a1628)"}}>
                            <div className="px-6 pt-6 pb-2 flex items-start justify-between">
                                <div>
                                    ...
                                    <p className="text-base font-bold text-white">Hydro · Biomass · Petroleum</p>
                                    <p className="text-[11px] mt-0.5" style={{color:"rgba(255,255,255,0.3)"}}>Tonnes of Oil Equivalent · 2010–2022</p>
                                </div>
                                <div className="flex gap-3 mt-1">
                                    {[["Hydro",P.teal],["Biomass","#65a30d"],["Petro",P.gold]].map(([n,c])=>(
                            ...
                            <ResponsiveContainer width="100%" height={250}>
                                <AreaChart data={TREND} margin={{left:5,right:10,top:5,bottom:10}}>
                                    ...
                                    <Area type="monotone" dataKey="petroleum" name="Petroleum" stroke={P.gold}  strokeWidth={2.5} fill="url(#tP)"/>
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                        {/* White bar chart */}
                    ...
                    {/* 2016→2022 delta cards */}
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        {[{l:"Hydro Supply",v22:301852,v16:222500,unit:"TOE",c:P.teal,ic:"💧"},{l:"Petroleum",v22:130000,v16:97500,unit:"TOE",c:"#b45309",ic:"🛢️"},{l:"Biomass",v22:100617,v16:84693,unit:"TOE",c:"#65a30d",ic:"🌿"},{l:"Installed Cap.",v22:2335,v16:1614,unit:"MW",c:"#2563eb",ic:"⚡"}].map(({l,v22,v16,unit,c,ic})=>{
            ...
            {/* SECTORS — original */}
            <section className="py-20 border-b border-slate-100" style={{background:"#f6fbf8"}}>
                <div className="max-w-[1320px] mx-auto px-6 lg:px-10">
            ...
            <section className="py-16 border-b border-slate-100" style={{background:"#f6fbf8"}}>
                <div className="max-w-5xl mx-auto px-6 text-center">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-4">
                        {INTEG.map(({a,f})=>(
                            <div key={a} className="flex flex-col items-center p-4 rounded-2xl border border-slate-100 bg-white hover:shadow-md hover:-translate-y-0.5 transition-all">
                                <div className="h-11 w-11 rounded-2xl mb-2 flex items-center justify-center text-white text-sm font-black" style={{background:`linear-gradient(135deg,${P.forest},${P.teal})`}}>{a[0]}</div>
                                <p className="font-extrabold text-slate-800 text-sm">{a}</p>
                                <p className="text-slate-400 text-[9px] mt-0.5 text-center leading-tight">{f}</p>
                            </div>
                        ))}
                    </div>
                </div>
