...
export const analyticsAPI = {
  getWidgets: () => apiFetch("/analytics/widgets/").then(r => r.json()),
  getDashboard: () => apiFetch("/analytics/dashboard/").then(r => r.json()),
  saveDashboard: (data) => apiFetch("/analytics/dashboard/", { method: "POST", body: JSON.stringify(data) }).then(r => r.json()),
  getDiscovery: () => apiFetch("/analytics/discovery/").then(r => r.json()),
  queryData: (params) => {
    const q = new URLSearchParams(params).toString();
    return apiFetch(`/analytics/query/?${q}`).then(r => r.json());
  },
};

export const reportingAPI = {
  getSummary: (year) => apiFetch(`/reporting/dashboard/summary/?year=${year}`).then(r => r.json()),
  getGHG: (year) => apiFetch(`/reporting/dashboard/ghg/?year=${year}`).then(r => r.json()),
  getGeneration: (year) => apiFetch(`/reporting/dashboard/generation/?year=${year}`).then(r => r.json()),
};
