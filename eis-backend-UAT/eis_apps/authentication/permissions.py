...
class IsSystemAdmin(BasePermission):
    ...
    def has_permission(self, request, view):
        return (
            ...
            and (get_user_role(request.user) == ADMIN or request.user.is_superuser)
        )
...
class IsSystemAdminOrDOEHead(BasePermission):
    ...
    def has_permission(self, request, view):
        return (
            request.user
            and request.user.is_authenticated
            and (get_user_role(request.user) in [ADMIN, DOE_HEAD] or request.user.is_superuser)
        )
...
class IsDataManagerOrAbove(BasePermission):
    ...
    def has_permission(self, request, view):
        return (
            request.user
            and request.user.is_authenticated
            and (get_user_role(request.user) in [ADMIN, DOE_HEAD, DATA_MANAGER] or request.user.is_superuser)
        )
...
class IsDataFocalOrAbove(BasePermission):
    ...
    def has_permission(self, request, view):
        return (
            request.user
            and request.user.is_authenticated
            and (get_user_role(request.user) in [ADMIN, DOE_HEAD, DATA_MANAGER, DATA_FOCAL] or request.user.is_superuser)
        )


class IsViewerOrAbove(BasePermission):
    """Any authenticated user with a valid role (including Viewer)."""
    def has_permission(self, request, view):
        return (
            request.user
            and request.user.is_authenticated
            and (get_user_role(request.user) in [ADMIN, DOE_HEAD, DATA_MANAGER, DATA_FOCAL, VIEWER] or request.user.is_superuser)
        )
