...
class LoginView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        max_attempts = setting.max_login_attempts
        ...
        if attempts >= max_attempts:
            ...
            lockout_key = f"login_lockout:{username}:{ip}"
            if cache.get(lockout_key):
                return Response({
                    "detail": f"This account is temporarily locked due to too many failed attempts. Please try again in 30 minutes."
                }, status=status.HTTP_403_FORBIDDEN)
            else:
                # Cache expired, reset attempts
        ...
        try:
            ...
            if attempts >= max_attempts:
                ...
                log_action(None, AuditLog.Action.LOCKED, request, f"Account locked: {username}")
                return Response({
                    "detail": "Too many failed attempts. Account locked for 30 minutes."
                }, status=status.HTTP_403_FORBIDDEN)
            
            # Record failed login in audit log
            log_action(None, AuditLog.Action.LOGIN_FAILED, request, f"Failed login attempt: {username}")
            raise e
...
class UserSessionListView(generics.ListAPIView):
    """Admin-only view to list all active sessions."""
    ...
    def get_queryset(self):
        # only show active and non-expired sessions
...
class UserSessionDeleteView(APIView):
    """Admin-only view to revoke a specific session."""
    permission_classes = [IsAuthenticated, IsSystemAdmin]
    ...
    def delete(self, request, pk):
        try:
            ...
            session.save()
            log_action(request.user, "SESSION_REVOKED", request, 
                       f"Revoked session for {session.user.username} (IP: {session.ip_address})")
            return Response({"detail": "Session revoked successfully."}, status=200)
